...
> Reference: `start_plan.md` + locked decisions (system C, optimization B, Bambu Studio, PLA+PETG, `core/`, `docs/`).

**Root problem:** no repeatable SOP or linked corpus; optimization lived in ad-hoc chat memory.  
**Target architecture:** neural wiki in `docs/projeto` + `playbook.md` (decisions) + `core/` (inspect/validate ports; trimesh/zip adapters) + I/O `3ds/` + `plan/`.
