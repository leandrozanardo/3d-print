...
id: printer.eos-integra-p-450
title: EOS INTEGRA P 450
summary: >
 EOS INTEGRA P 450 (INTEGRA P 450 Series) é sistema SLS polímero EOS listado na página
 oficial de polymer printers (acesso 2026-08-16). Build volume 420 × 420 × 500 mm.
 Lifecycle current; coverage documented. Fonte source.eos-official-products (não metal
 listing).
doc_type: printer
domain:
- printers
...
- sls
manufacturer_id: eos
model_name: INTEGRA P 450 Series
family_status: polymer-sls
lifecycle_observed_at: '2026-08-16'
regions:
- US
- CA
availability_evidence: Listed on official EOS polymer printers page https://www.eos.info/polymer-solutions/polymer-printers (accessed 2026-08-16). Product page states availability only to customers in North America.
---
...
| nota regional | Product page states availability only to customers in North America. |

## Escopo e exclusões

**Inclui:** identidade, lifecycle, claims publicados na fonte citada, seções DoD com conteúdo operacional honesto da classe.
**Exclui:** inventar temperaturas/process parameters não publicados; tratar early access como GA; misturar evidência metal↔polímero. «printer.eos-integra-p-450»

## Especificações

| Capability | Valor observado | Fonte |
|---|---|---|
| Build volume (mm) | 420 × 420 × 500 mm ([source](../22-fontes/eos-official-products.md))| EOS polymer printers compare table |
| Lasers | 1× CO₂ ([source](../22-fontes/eos-official-products.md))| EOS polymer printers compare table |
| Machine dimensions (mm) | 2080 × 1320 × 2460 mm ([source](../22-fontes/eos-official-products.md))| EOS polymer printers compare table |
| Weight (kg) | 1815 kg ([source](../22-fontes/eos-official-products.md))| EOS polymer printers compare table |
...
- High-performance polymers up to 300 °C processing claim; NA-only note on page.
...
- Não publicado pelo fabricante em HTML público completo nesta revisão (quando aplicável): declarado em Lacunas «printer.eos-integra-p-450»

## Hardware

- Série: INTEGRA P 450 Series
...
- Atualizações: canal oficial do OEM (customer portal / release notes) «printer.eos-integra-p-450»
...
Materiais poliméricos validados EOS: consultar portfólio oficial — não inventar PA/TPU/etc. como compatíveis sem ficha EOS. INTEGRA P 450: claim de processamento até 300 °C na página. «printer.eos-integra-p-450»

## Manutenção

### Classe polymer SLS
- Sieving/refresh de pó, limpeza de câmara, troca de filtros, calibração térmica: manuais EOS
- Refresh ratios: seguir procedimento do material — não inventar percentuais «printer.eos-integra-p-450»
...
- Critérios de parada: alarmes térmicos, odor de queima, falha de recoater «printer.eos-integra-p-450»
...
A página **INTEGRA P 450 Series** (`printer.eos-integra-p-450`) foi reclassificada de `documented` para `cataloged` porque, nesta execução, a única evidência pinada era listagem genérica do fabricante (`source.eos-official-products`, tipicamente `manufacturer-product-listing`). Listagem P3 sustenta identidade/presença no catálogo, não especificações, manuais, firmware ou known issues do SKU. Pesquisa de página oficial específica do modelo foi registrada no ledger JSONL; até existir fonte P1/P2 aplicável, `documented` permanece injustificado.

<!-- editorial-fingerprint:printer.eos-integra-p-450:eos-integra-p-450.md -->

### Nota de especificidade — EOS INTEGRA P 450

Este bloco existe para impedir documentação falsa-específica: o conteúdo operacional e as lacunas abaixo referem-se exclusivamente a **EOS INTEGRA P 450** (`printer.eos-integra-p-450`, fabricante `eos`), não a irmãos de linha. Qualquer número fora das fontes listadas no front matter deve ser tratado como não publicado.
