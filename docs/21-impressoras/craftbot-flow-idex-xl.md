...
model_name: Craftbot Flow IDEX XL
...
Para **Craftbot Flow IDEX XL**, o foco operacional desta base é identidade + lifecycle observado + classe material-extrusion/fff.
...
| Modelo | Craftbot Flow IDEX XL |
| Título canônico | Craftbot Flow IDEX XL |
| Família | unknown |
| Aliases EN | Craftbot Flow IDEX XL, Craftbot Flow IDEX XL |
...
| nome observado | Craftbot Flow IDEX XL / Craftbot Flow IDEX XL |
...
**Inclui:** identidade de **Craftbot Flow IDEX XL**, evidência de lifecycle acima, classificação material-extrusion/fff, procedimentos editoriais e lacunas específicas do SKU.
...
Não reutilizar números de outras máquinas craftbot como se fossem de **Craftbot Flow IDEX XL**.
...
Mapa de manuais específicos de **Craftbot Flow IDEX XL**: não publicado / não localizado na superfície citada em 2026-08-16. Próximo passo: portal de suporte craftbot + PDF por SKU.

## Hardware

Hardware detalhado (eixos, hotend/óptica/energia, sensores, revisões) de **Craftbot Flow IDEX XL**: **não publicado** na evidência de listagem. Registrar apenas declarações da página de produto específica em revisão futura.

## Software

Para **Craftbot Flow IDEX XL**, não assumir stack de software de outro produto craftbot.

## Firmware

Atualização de firmware de **Craftbot Flow IDEX XL** exige fonte OEM dedicada antes de qualquer claim de versão.

## Slicer

Usar perfis oficiais craftbot quando existirem para **Craftbot Flow IDEX XL**. Não colar perfil de irmão de linha sem revisão.

Calibração típica da classe: nivelamento/Z-offset, flow e temperaturas por material — não copiar presets de outro modelo.

## Materiais

Compatibilidade Ideal/Capable/Not Recommended de **Craftbot Flow IDEX XL**: **não publicada** na listagem genérica citada. Exigir TDS/página de produto antes de recomendar polímero/resina/pó de engenharia.

## Manutenção

Para **Craftbot Flow IDEX XL**: verificar path de filamento, bico, mesa/adesão e lubrificação de eixos conforme manual OEM. Intervalos/torque específicos: somente manual oficial.

## Segurança

Riscos de classe para **Craftbot Flow IDEX XL**: superfícies quentes, partes móveis, risco de blob/incêndio residual se impressão desacompanhada sem monitoramento.
...
Pesquisa de recall/service bulletin para **Craftbot Flow IDEX XL** (craftbot) não retornou evidência suficiente para promover issue conhecida.
...
- Firmware/release notes pinados para **Craftbot Flow IDEX XL**
- Matriz de materiais Ideal/Capable/Not Recommended publicada para **Craftbot Flow IDEX XL**
- Troubleshooting oficial por sintoma para **Craftbot Flow IDEX XL**
