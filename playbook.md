...
- System **C (hybrid)**: wiki + agent decide; Node CLI inspects/repairs lightly
...
2. **File inventory** — `fix-my-print inspect-mesh|inspect-3mf … --json`
...
npx --yes pnpm@10.12.1 --filter @fix-my-print/cli exec node dist/bin.js inspect-mesh path.stl --json
npx --yes pnpm@10.12.1 --filter @fix-my-print/cli exec node dist/bin.js inspect-3mf path.3mf --json
npx --yes pnpm@10.12.1 --filter @fix-my-print/cli exec node dist/bin.js repair-mesh in.stl out.stl --json
```
